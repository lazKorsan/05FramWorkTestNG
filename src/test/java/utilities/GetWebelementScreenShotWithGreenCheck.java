...
public class GetWebelementScreenShotWithGreenCheck {

    private static final String SCREENSHOT_DIR = "C:\\Users\\Hp\\OneDrive\\Desktop\\loyalfriendcare\\";

    /**
     * Belirtilen butonun ekran görüntüsünü alır ve altına yeşil onay işareti (✓) ekler.
     *
     ...
     */
    public static String captureButtonWithGreenCheck(WebElement button, String buttonName) {
        ...
        try {
            ...
            // 4. Butonun bölgesini kırp (onay işareti için ekstra alan ekle)
            int checkHeight = 40; // Yeşil onay işaretinin yüksekliği için ekstra alan
            BufferedImage buttonImage = fullImage.getSubimage(
                    x.intValue(),
                    y.intValue(),
                    width.intValue(),
                    height.intValue() + checkHeight); // Buton + onay işareti alanı

            // 5. Yeşil onay işareti (✓) çiz
            Graphics2D g = buttonImage.createGraphics();
            g.setColor(Color.GREEN);
            g.setStroke(new BasicStroke(5)); // Onay işaretinin kalınlığı

            // Onay işaretinin pozisyonu (butonun altına)
            int arrowX = width.intValue() / 2; // Butonun ortası
            int arrowY = height.intValue() + 20; // Butonun 20px altı
            ...
            String fileName = (buttonName == null || buttonName.trim().isEmpty() ? "button_screenshot" : buttonName)
                    + "_check_" + System.currentTimeMillis() + ".png";
